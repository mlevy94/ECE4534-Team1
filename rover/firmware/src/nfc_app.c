...
static NFC_APP_DATA nfc_appData;
...
BaseType_t addToNFCQ(InternalMessage msg) {
    return xQueueSend(nfc_appData.inQ, &msg, portMAX_DELAY);
}

BaseType_t addToNFCQFromISR(InternalMessage msg) {
...
bool getACK() {
    char inbyte;
    // get start bytes
    sendDebugMessage("GETTING ACK\0");
    while(1) {
...
void NFC_APP_Tasks ( void )
{
    ...
    while(1) {
        ...
        sendDebugMessage("GET ACK\0");
        if (getCheckNFC()) {
            addToUartTXQ(tokenFound());
        }
        sendDebugMessage("GET NFC\0");
