...
*/
// Passes message from usart ISR to queue
BaseType_t addToUsartRxQFromISR(char* val){
 #ifdef DEBUG_ON
    setDebugVal(ENTER_ADDTOUSARTRXQFROMISR);
#endif
    return xQueueSendFromISR(usart_rxData.usart_rxQ, val, 0);
}
...
void USART_RX_Initialize ( void )
{
     ...
     * parameters.
     */
    usart_rxData.usart_rxQ = xQueueCreate(32, 8); // Creating Queue
}
...
void USART_RX_Tasks ( void )
{
    ...
    int i = 0;
    while(1){
        setDebugBool(DRV_USART0_ReceiverBufferIsEmpty());
        // clear message to 0
        ...
        //addToAppRXQ(usart_rxData.in_msg); // This was left here not by CHRIS
    }
}
